...
Forventede miljøvariabler (må settes før kjøring, men GH_ORG/GH_REPO autodetekteres fra git remote hvis de ikke er satt):
  export AWS_IAC_ROLE_ARN="arn:aws:iam::<ACCOUNT_ID>:role/MathVisualsGithubDeploy"
  # valgfritt: export GH_ORG="<github-org>"
  # valgfritt: export GH_REPO="<github-repo>"
...
if [[ -z "${GH_ORG:-}" || -z "${GH_REPO:-}" ]]; then
  if [[ "$REMOTE_URL" =~ github.com[:/]+([^/]+)/([^/.]+)(\.git)?$ ]]; then
    GH_ORG="${GH_ORG:-${BASH_REMATCH[1]}}"
    GH_REPO="${GH_REPO:-${BASH_REMATCH[2]}}"
    echo "Fant repo fra git remote: ${GH_ORG}/${GH_REPO}"
  else
    [[ -z "${GH_ORG:-}" ]] && echo "Missing env: GH_ORG (export it before running)." >&2
    [[ -z "${GH_REPO:-}" ]] && echo "Missing env: GH_REPO (export it before running)." >&2
    missing=1
  fi
fi

if [[ -z "${AWS_IAC_ROLE_ARN:-}" ]]; then
  echo "Missing env: AWS_IAC_ROLE_ARN (export it before running)." >&2
  missing=1
fi
...
ACTUAL_SUB=$(jq -r '
  ...
  | .Condition.StringLike."token.actions.githubusercontent.com:sub"? // empty' "${ROLE_JSON}" | head -n1)
...
if [[ "${SUB_MATCH}" != "true" ]]; then
  echo "❌ sub condition does not match ${EXPECTED_SUB} (actual: ${ACTUAL_SUB:-<unset>})"
  TRUST_OK=0
else
...
if [[ "${TRUST_OK}" -eq 1 ]]; then
  ...
  echo "Tip: Kjør REPO_NAME=\"${GH_ORG}/${GH_REPO}\" BRANCH_PATTERN=\"${BRANCH_PATTERN}\" bash scripts/cloudshell-create-oidc-role.sh for å oppdatere rollen."
fi
