...
(function initMeasurementApp() {
  ...
  attachInstrumentPointerHandlers(tapeHousingShiftHandle, 'tape');
  ...
  function renderTapeMeasureStrap(settings, unitSpacing, scaleMetrics) {
    ...
    const zeroHandleWidth = Math.max(0, Math.min(strapEndWidth, strapLengthWithOverlap));
    if (tapeMeasure) {
      tapeMeasure.style.setProperty('--tape-zero-handle-width', `${zeroHandleWidth}px`);
    }
